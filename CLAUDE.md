...
- `python/` - Python project (hatchling), hand-written client using `requests`
- `openapi.yaml` - Shared OpenAPI 3.1.0 spec at repo root
- Java and .NET projects use awk preprocessing to simplify `manufacturerId` oneOf → plain string
...
- Java and .NET artifacts publish to GitHub Packages (not Maven Central / NuGet.org)
- Python wheels are attached to GitHub Releases (GitHub Packages has no PyPI registry)
